...
public class GameManager : MonoBehaviour
{
    ...
    IEnumerator TransitionToScene(string sceneName)
    {
        ...
        // Find the currently active gameplay scene (anything that isn't _Bootstrap)
        Scene currentGameplay = default;
        ...
        // Unload current gameplay scene if found
        if (currentGameplay.IsValid())
            yield return SceneManager.UnloadSceneAsync(currentGameplay);

        // Load new gameplay scene additively (bootstrap stays loaded underneath)
        yield return SceneManager.LoadSceneAsync(sceneName, LoadSceneMode.Additive);

        // Set new scene as active so lighting/skybox use its settings, not bootstrap's
    ...
    IEnumerator Fade(float from, float to)
    {
        ...
        Debug.Log($"[Fade] from={from} to={to}, fadeCanvas found={fadeCanvas != null}");
        ...
        Debug.Log($"[Fade] done, final alpha should be {to}");
    }
