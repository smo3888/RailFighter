...
public class PowerUpManager : MonoBehaviour
{
    ...
    public void ActivateDefensive()
    {
        ...
        // Passives have no Q/E activation — the effect is always on while equipped.
        if (equippedDefensive.effectType == PowerUpEffectType.MagnetRail) return;
    ...
    // ── Passive sync ─────────────────────────────────────────────────────────
    // For passive defensive powerups (e.g. MagnetRail), being equipped is what
    // turns the effect on. This method pushes the current state to the player.
    void ApplyPassiveDefensiveState()
    {
        PlayerControllerRailFighter player = FindObjectOfType<PlayerControllerRailFighter>();
        if (player == null) return;

        bool magnetEquipped = equippedDefensive != null
            && equippedDefensive.effectType == PowerUpEffectType.MagnetRail;
        player.magnetFlightActive = magnetEquipped;
    }
